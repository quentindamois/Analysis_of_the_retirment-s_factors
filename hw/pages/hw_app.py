...
#st.sidebar.markdown("visualistion of the second dataset")
...
#st.title("visualistion of the second dataset")
...
with col1:
    xAxis = st.selectbox(label="Select x :", options=("passenger_count", "trip_distance", "fare_amount", "tips_amount", "total_amount"))
   
with col2:
    yAxis = st.selectbox(label="Select y :", options=("passenger_count", "trip_distance", "fare_amount", "tips_amount", "total_amount"))
...
temp_data = data2.groupby("passenger_count").mean()


plot = px.Figure(data=[px.Scatter(
    y=temp_data["total_amount"],
    mode='lines',)
])
plot.update_layout(
    xaxis=dict(
